...
const { claudeNoopDecision, claudePermissionRequestDecision } = require("../shared/protocol");
...
function writeDecision(decision) {
  process.stdout.write(JSON.stringify(claudePermissionRequestDecision(decision)));
}
...
async function main() {
  ...
  try {
    ...
    writeDecision({
      behavior: "deny",
      message: `Invalid Claude Code PermissionRequest JSON: ${error.message}`
    });
    return;
  }
  ...
  try {
    const response = await postJson("/hook/permission-request", input);
    ...
    writeDecision({
      behavior: "deny",
      message: `Companion daemon unavailable. Start it with npm run daemon. Details: ${error.message}`
    });
  }
}
...
main().catch((error) => {
  writeDecision({
    behavior: "deny",
    message: `Companion PermissionRequest hook failed: ${error.message}`
  });
});
