...
<p align="center"><em>后台守着你的 Claude Code 智能体；第二天，把它教你的东西复习一遍。</em></p>
...
`setup-user-hooks` 写的 JSON 条目直接 POST 到 `http://127.0.0.1:4317/hook/<endpoint>`，没有本地 hook 脚本 —— Claude Code 用原生 HTTP 通道直接和 daemon 对话。这意味着 daemon 关掉、Vibedog-for-agents 卸了之类的情况下，Claude Code 把它当成"非阻断错误"记一条 log 然后正常继续，**不会**像 fail-closed 包装脚本那样把工具调用卡死。每条条目带一个 `"x-clawdeck-version"` 字段，未来版本可以检测+原地升级自己的条目。
...
打包好的 `Vibedog-for-agents.exe`第一次启动时自己就把 hook 装上，NSIS 卸载器在删 .exe 前会先调 `--uninstall-hooks` —— 手动跑 `setup-user-hooks` 只在 dev 场景下需要。
